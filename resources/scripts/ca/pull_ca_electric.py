...
def parse_rows(html: str, source_tag: str):
    ...
    # Prefer cards inside a grid container, but fall back to any <a.group>
    cards = soup.select("div.grid a.group[href]") or soup.select("a.group[href]")

    for a in cards:
        href = (a.get("href") or "").strip()
        # Name from the card heading <h2> if present; else anchor text
        h2 = a.find("h2")
        ...
        if href:
            ...
            # Fallback: sometimes the visible domain is in a trailing <p>
            # e.g., <p>torontohydro.com</p>
            tail_p = a.find_all("p")
            if tail_p:
                url = tail_p[-1].get_text(" ", strip=True)
...
def main():
    ap = argparse.ArgumentParser(description="Parse Electricity Canada grid cards into CSV (name,url,abbr,source,).")
    ...
    if not rows:
        sys.stderr.write("No rows parsed. Ensure the HTML contains <a class='group' href='...'><h2>...</h2> ...</a> cards.\n")
        sys.exit(2)
