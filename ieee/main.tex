...
% TODO
  % - add frame-x-options block.
  % - format tables
  % - elaborate on the fact that origin eval happened at input and final, endpoint final-only
  % - remove talk on cipher suites, or add it back to the TLS summary table
  % - confirm methods mentions checking for contact and expires in security.txt
...
We curated a sector-balanced dataset across Canada (CA), the United States (US), and the
United Kingdom (UK), spanning government, finance, education, and energy sectors. 
Candidate websites were sourced from authoritative registries and well-established sector lists to
prioritize institutional relevance and reduce sampling bias. 
We distinguish between \emph{origins} (scheme + host + port) and \emph{endpoints} (origins with
specific paths), enabling measurements at both transport and application levels.


% We have 4 sectors of websites we performed evaluation on for each country. 
% \emph{Authorities}, consist of government 
...
All of our origin-targeting tests (e.g., cryptographic tests)  were performed on both the final and entry origin. 
...
eligibility requires \texttt{max-age} $\ge 31536000$ seconds (1 year) \cite{mdn_sts_header,chromium_hsts_preload,firefox_hsts_preload}.
...
\cite{owasp_http_security_response_headers,mdn_csp_guide,chrome_permissions_policy,owasp_csp_cheatsheet,gds_security_overview_websites, owasp_clickjacking_defense}. 
...
\texttt{Content-Security-Policy} with \texttt{frame-ancestors}, \texttt{X-Content-Type-Options}=\texttt{nosniff},
 and \texttt{Permissions-Policy} \cite{owasp_clickjacking_defense}. The \texttt{Referrer-Policy} header controls how much of the current page's URL (the ``referrer'') is sent 
along when a browser navigates to another site or loads third-party content. \texttt{Content-Security-Policy} (CSP) aims at reducing the 
attack surface of cross-site scripting (XSS) and other content-injection attacks by restricting which 
sources of scripts, images, and other resources the browser may load. The \texttt{X-Content-Type-Options} header eliminates 
...
We assess cookie hardening because weak cookie attributes can undermine session security and increase exposure to common
web attacks<CITE SOME HERE>. The scanner parses the following cookie parameters inline with modern guidance \cite{cccs_itsp_40_111,mdn_secure_cookie_configuration}.
...
protocol and excluded from subsequent modules. Thus, preventing repeated probing of unstable services. All measurements 
 relied on standard HTTP(S) requests and redirects only, without any exploit payloads.

% \begin{table}[t]
%   \centering
%   \caption{Scanner rate limits and timeouts (non-invasive configuration).}
%   \label{tab:scan_limits}
%   \small
%   \setlength{\tabcolsep}{5pt}
%   \begin{tabular}{lccc}
%   \toprule
%   \textbf{Category} & \multicolumn{2}{c}{\textbf{Concurrency limits}} & \textbf{Timeout} \\
%   \cmidrule(lr){2-3}
%    & \textbf{Global} & \textbf{Per-origin} &  \\
%   \midrule
%   Authorities & 15 & 3 & 3\,s \\
%   Edu/Fin/Energy & 20 & 5 & 3\,s \\
%   \bottomrule
%   \end{tabular}
%   \end{table}
...
The table below illustrates the outcome of our initial resolution stage. 
...
resolved hosts. Finally, the 2xx status column indicates the percentage of redirects
that ended in a positve status code.
...
\begin{table}[t]
  ...
  \csvreader[
    ...
    table head=\toprule
      \textbf{Dataset} &
      \multicolumn{2}{c}{\textbf{\emph{n} URIs}} &
      \multicolumn{2}{c}{\textbf{\emph{n} Origins}} &
      \textbf{Final 2xx (\%)} \\
      \cmidrule(lr){2-3}\cmidrule(lr){4-5}
       & \textbf{Input} & \textbf{Final} & \textbf{Input} & \textbf{Unique} & \\
      \midrule,
    late after line=\\,
    ...
    \Nfinal &
    \NorigIn &
    % --- unique origins (hardcoded, CSV unchanged)
...
During the resolution pass, final 2xx resolution rates varied substantially across datasets, 
with CA Authorities exhibiting the lowest success rate 
(68.4\%) compared to 80–95\% in most other sectors.
  
\subsection{Website Security Evaluation}

\subsubsection{Transport Security}

The following two tables (insert ref) summarize the results of the HTTP/HTTPS and HSTS evaluation.
Notably, many sectors exhibit low percentages
within the \emph{strong} HSTS bucket (at least one-year max-age with includeSubDomains). 
Additionally, one should note that ``unreachable'' here is \emph{not} 
an indicator that the site is offline. Rather, that
our scanner was repeatedly unable to complete a TLS handshake or receive an HTTP response. 
Any resulting percentages were only recorded on those sites that did not timeout, or become 
disabled by the scanner's rate limits. 
...
% - HSTS
% - shares of sites columns

The following table summarizes TLS protocol support, and naturally negotiated versions all datasets for each country and sector. 
...
\vspace{1em}The next table reports security.txt adoption and field completeness by country and sector. For each origin, the 
scanner searches standard locations (/.well-known/security.txt and /security.txt) for the presence of a security.txt 
file per RFC 9116. 
% \begin{inlinecap}
...
\begin{table}[t]
  ...
  \centering
  \scriptsize
  \setlength{\tabcolsep}{2.2pt} % tighter spacing
  \renewcommand{\arraystretch}{1.15}
...
The following 2 tables present security header adoption and 
