...
performs the aggregation and evaluation of Canadian authorities websites against Canadian specific webs security 
...
Note that we UK energy providers were excluded due to the absence of an authoritative source 
with usable URIs. As such leave our energy site comparison between Canada and the US.
...
consolidated by internation authorities and browser engines. For example: the National Security Agency (NSA), 
...
other resources the browser may load. Providing additional security over the aformentioned 
...
no stack traces or banners were reported, thus guarding against false positives. Similar to the previous tests, we then configured small local web servers return known responses on specific paths, including pages that 
...
\cite{dhs-bod-18-01,omb-m-15-13}. By contrast, 
...
where CCCSA recommendations have been followed, Canadian sites tend to implement them well. However, the contrast 
