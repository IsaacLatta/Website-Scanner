...
Canadians interact with and depend on daily. To provide a refrence points 
...
We have 4 \emph{buckets} of websites we performed evaluation on for each country. \emph{Authorities}, consist of government 
...
We compiled a very similar set of URIs for American authorities, financial and education institutions, and energy companies. The United States Government hosts an index into
...
USA education institutions were retrieved from the US Department of Education's ``College Scorecard'' API \cite{us-college-scorecard-docs}. We once again took the top 10 states by population, then used queried the API
for the top 5 education institutions by population. For US Energy, we leveraged Edision Electric's list of 
member companies websites \cite{edison-electric-member-sites}.

% \subsubsection{United Kingdom}

The UK maintains a authoritative of list of their government organizations \cite{uk-gov-organisations}, 
this list is also served via the UK Government's ``Collections API'' \cite{uk-gov-collections-api-docs}.
We used this Collections API as our source for UK government URIs. We pulled all pages from the collections endpoint 
\cite{uk-gov-collections-api}, 
...
the list of regulated banks FRNs from the PRA list. This was done as the PRA list is a subset of the FCA API's list, cross referencing 
ensured our list was composed 
of \emph{only} UK banks, not the surplus of financial institutions present in the FCA API. Educational Institutions of the 
UK were collected from the Government's ``Discover Uni'' dataset \cite{hesa_discoveruni_2024}. This dataset hosts the 
names and URLs of each university in the United Kingdom. Lastly, no UK energy companies were 
included in this scan. Unfortunately, we were unable to find a well-backed and authoritative list that contained useable 
links for our scanner. As such, we chose to omit this sector from the UK, and instead leave our energy site comparison between Canada and the US.
...
\vspace{1em}The first evaluation tests whether each origin actually supports HTTPS and whether it enforces HTTPS 
in line with modern guidance. This is is an origin-scoped test, as HTTPS/HTTP operate at the transport layer. 
...
The goal of these checks was two answer practical questions, do public-facing services offer modern TLS versions, 
...
\vspace{1em}Additionally, our scanner also performs a series of presence and evalution checks on set of security headers. These checks are performed on 
each of the final URIs (URI-scoped). The first header we probe for is \texttt{Referrer-Policy}.
...
both \cite{owasp_clickjacking_defense}. For each site, we check whether it sends a CSP that restricts 
...
\vspace{1em}The next header the scanner evaluates is the \texttt{Permissions-Policy} (formerly \texttt{Feature-Policy}). This header allows a site
...
\vspace{1em}Beyond checking for the presence of modern security headers, we also examined whether sites leaked unnecessary implementation details through so--called 
...
Misconfigured error pages are another common source of unintended information disclosure. These error pages can reveal the exact framework, cloud provider, or 
...
on the final origins. To assess this risk in a safe and consistent way, we carried out a
...
jurisdictions (Canada, US, UK). The results reveal both strengths and critical gaps in Canada's public-sector web 
security posture.
...
\vspace{1em}Canadian authorities demonstrate mixed compliance with these standards. For transport security, 76\% of 
Canadian authority origins successfully served HTTPS, with 90\% of HTTP-responding origins correctly 
redirecting to HTTPS. However, only 59.7\% of HTTPS-capable sites deployed HSTS, and of those, merely 46.3\% 
implemented strong policies meeting the one-year \texttt{max-age} threshold with subdomain coverage. 
This leaves a substantial portion of authority sites vulnerable to SSL-stripping attacks.

\vspace{1em}TLS configuration presents additional concerns. While 94.4\% of Canadian authorities support 
TLS~1.2, only 65.9\% advertise TLS~1.3 support. More troubling, 11.5\% still accept TLS~1.1 and 9.6\% accept 
TLS~1.0, contrary to CCCS deprecation guidance. In natural handshakes, 69.1\% negotiated TLS~1.3, with 74.9\% 
using recommended cipher suites when forced to TLS~1.2.

\vspace{1em}Security header adoption remains inconsistent. Canadian authorities achieved 62.3\% adoption for 
\texttt{X-Content-Type-Options} and 63.5\% for \texttt{X-Frame-Options}, but only 17.9\% implemented CSP 
\texttt{frame-ancestors}, 17.4\% set \texttt{Referrer-Policy}, and a mere 8.6\% deployed \texttt{Permissions-Policy}. 
Cookie security is similarly weak: among the sites setting cookies, only 63.5\% used the \texttt{Secure} 
flag, 48.7\% used \texttt{HttpOnly}, and just 3.5\% achieved recommended configurations with all three protective 
attributes.

\vspace{1em}Vulnerability disclosure mechanisms are nearly absent. Only 1.1\% of Canadian authority origins 
(4 of 375) published a \texttt{security.txt} file, and of these, only 25\% maintained valid expiration dates. 
This represents a significant barrier to responsible disclosure of security vulnerabilities.

\vspace{1em}Information leakage is pervasive. Technology-revealing headers appeared on 94.4\% of Canadian 
authority origins, with 51.2\% exposing one header and 32.4\% exposing two. Error page analysis found 47.2\% 
leaked technology information, with 10.7\% exposing specific version numbers that could facilitate targeted 
exploitation.

\vspace{1em}Cross-sector analysis reveals that Canadian authorities generally underperform relative to 
other Canadian sectors and international counterparts. Canadian finance leads TLS configuration domestically 
with 94.4\% TLS~1.3 support and 89.6\% recommended cipher usage, substantially outperforming Canadian 
authorities (65.9\% and 74.9\%, respectively). Canadian education (77.7\% TLS~1.3, 80.4\% recommended ciphers) and 
energy (84.1\% TLS~1.3, 68.1\% recommended ciphers) occupy intermediate positions. Internationally, US authorities 
demonstrate superior TLS~1.3 adoption at 83.9\% compared to Canadian authorities' 65.9\%, with higher recommended 
cipher usage (85.1\% versus 74.9\%) and lower legacy protocol support.

\vspace{1em}Canadian authorities' 59.7\% HSTS adoption significantly trails US authorities' 89.7\%. Among 
Canadian sectors, finance achieves the highest HSTS presence at 78.6\%, though with lower strong policy 
rates (34.9\%) than authorities (46.3\%). US authorities not only lead in adoption but also in policy quality, 
with 64.9\% deploying strong configurations.

\vspace{1em}US authorities consistently outperform Canadian authorities across all header categories: 
78.0\% versus 62.3\% for \texttt{X-Content-Type-Options}, 81.2\% versus 63.5\% for \texttt{X-Frame-Options}, 
and 34.7\% versus 17.9\% for CSP \texttt{frame-ancestors}. Canadian energy sites demonstrate the highest domestic 
\texttt{Referrer-Policy} adoption at 23.7\%, compared to authorities' 17.4\%. UK finance sites lead in 
\texttt{Permissions-Policy} adoption at 24.0\%.

\vspace{1em}Security.txt adoption is uniformly poor across all datasets, but Canadian authorities' 
1.1\% adoption rate falls below US authorities (3.5\%), Canadian energy (4.2\%), Canadian education (3.9\%), 
UK education (5.8\%), and UK finance (12.1\%). This represents a critical gap in Canada's security posture.

\vspace{1em}Canadian finance demonstrates the best domestic practice, with only 53.5\% of origins revealing 
technology information compared to 94.4\% for Canadian authorities. US authorities achieve better information 
hygiene at 80.6\% disclosure. For error page leakage, Canadian finance again leads domestically at 20.9\% technology
 disclosure versus 47.2\% for authorities.
...
% evolution, deeper investigation of provincial and municipal government sites, assessment of authentication and 
% access control mechanisms, and development of a Canadian-specific web security scoring framework aligned with 
% CSA and CCCS standards. Additionally, examining the correlation between security posture and organizational factors 
% (budget, IT staffing, outsourcing arrangements) would inform policy development.
