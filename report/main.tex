...
% -------------------------------------------------------------------
% COPYRIGHT BLOCK — placed at bottom of title page
% -------------------------------------------------------------------
\vfill
\begin{center}
\small
...
services. First, we compile representative datasets of 
authority, finance, education, and energy websites in Canada, the United States, and the United 
Kingdom. Second, we evaluate them using an automated scanner grounded in guidance from the Canadian Centre 
for Cyber Security (CCCS), OWASP, MDN, and international cybersecurity authority guidance. 
The scanner assesses HTTPS and HSTS 
deployment, TLS protocol and cipher configuration, key security and privacy headers, cookie
protections, technology-disclosing headers, and the presence of vulnerability disclosure files. 
Overall, Canadian sites have adopted foundational 
transport-layer controls, but advanced protections are uneven and frequently lag behind the other international 
and domestic sectors. Canadian finance generally demonstrates 
that strong, standards-aligned configurations are achievable in practice. While authority sites 
exhibit lower HTTPS coverage, slower TLS~1.3 adoption, weaker headers, and 
minimal vulnerability-disclosure channels. These findings suggest that Canada’s primary challenge is
inconsistent adherence to existing guidance rather than a lack of technical ability. 
The report concludes by outlining opportunities for clearer standards and baselines around web security 
providing a more secure web for Canadians.
...
Lastly \emph{Energy}, consists of oil, gas, and nuclear energy companies, representing 
...
consolodated by internation authorities and browser engines. For example: the National Security Agency (NSA), 
...
Each TLS handshake's outcome is crossferenced against these lists to provide additional insights beyond simply the version.  
...
other resources the browser may load. Providing additional security over the aforemntioned 
...
the \texttt{frame-ancestors} derective. This directive controls exactly which other origins are 
...
landing page. For the purposes of this study, cookies that are marked both \texttt{Secure} and \texttt{HttpOnly} and use a strict
same-site policy are classified as \emph{recommended}, 
...
\cite{owasp_a05_2021,owasp_error_handling,cwe_209}. These error pages can reveal the exact framework, cloud provider, or 
...
content. 
Otherwise, the origin was treated as not exposing any interpretable error page. The body of each textual error page was then scanned for two classes of
...
For the authority sites, we allowed a conservative 15 total concurrent connections, with at most 
3 connections to each origin at a time. For the other sectors (finance, education, energy) 
...
gracefully, 
and produced the expected results for the securityy headers and cookie flags. 
...
often with the preload flag set.
...
\vspace{1em}Transport Layer Security (TLS) configuration tells a similar story. While 94.4\% of Canadian 
authority origins support TLS~1.2, only 65.9\% offer TLS~1.3, the lowest TLS~1.3 adoption among authority 
datasets examined. At the same time, 11.5\% still accept TLS~1.1 and 9.6\% accept TLS~1.0, contrary to CCCS 
deprecation guidance. Natural handshakes negotiated TLS~1.3 in 69.1\% of cases, and when connections were 
forced down to TLS~1.2, 74.9\% of Canadian authority sites used recommended cipher suites. This suggests that 
administrators are capable of following specific cipher guidance, but adoption of TLS~1.3 and retirement 
of legacy protocols has not yet been uniformly enforced. Canadian finance demonstrates what is achievable: 
94.4\% TLS~1.3 support and 89.6\% recommended cipher usage, substantially outperforming Canadian authorities.
...
lower adoption rate than every other dataset examined. 
...
disclosure policy on a separate origin (such as the UK central government domain) may be undercounted. Regardless
...
exposing two or more such headers (Table~\ref{tab:revealing_headers_summary}). Thus indicating that many sites disclose multiple layers of their tech stack (e.g., web 
server and application framework). Canadian education and energy sectors also 
show high rates of 
revealing headers, while Canadian finance again demonstrates comparatively good hygiene, with only 53.5\% of 
...
numbers \ref{tab:error_leaks_summary}. These rates are higher than those observed for most US and UK sectors, 
...
\vspace{1em}Taken together, these results suggest that Canada has implemented many of the controls 
identified in CCCS guidance; HTTPS, modern TLS versions, and some HSTS deployment. Canada still often falls short on the 
systematic HSTS preloading, full deprecation of legacy TLS, modern security and privacy 
headers, hardened cookies, minimal technology disclosure, and standardized vulnerability disclosure channels. 
Addressing these gaps would move Canadian authorities closer to international best practice and provide a more 
coherent, defence-in-depth web security posture for Canadian users.
