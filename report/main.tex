...
Often, when researchers discover security vulnerabilities, proper reporting channels are lacking,
 leading to vulnerabilities remaining unreported.
...
normalized URIs to be resolved. Following the normalization, we then performed a redirect resolution pass.
 Where the scanner sends a \texttt{GET} request and examines the response.
...
each hop the scanner records the HTTP status and target URL. After performing this resolution, we cached the final and
...
yields 2 final sets of URLs and origins for analysis, consisting of the entry and final URL-origin pair. In the results, we
 discuss our set of analysis modules in terms of their scope, 
i.e. whether they target an origin or the request URI. All of our tests origin-targeting tests (e.g., cryptographic tests) 
...
\vspace{1em}For each target in our normalized and resolved list. We then performed a series of \emph{tests} on each target. 
As mentioned earlier some tests target origins, 
while others target URIs. Because different information becomes available at different stages of the network stack, some checks must
 occur at the origin level while others 
occur at the URI level. Cryptographic properties are negotiated before any HTTP data is sent, whereas headers and
 resource-specific behavior can only be evaluated after the 
...
% \textbf{Goal is to answer these questions}
...
% \paragraph*{TODO: Add this discussion. Basic skeleton below.}

% As outlined by the CCCS, there is a minimal set of requirements a website should have. 
% A website should have atleast implement HTTP to HTTPS redirection and also implement HSTS.
% Servers implementing TLS should also adhere to the recommended cipher suites found in the 
% CCCS ``recommended'' category, and if need for compatability may include those found in ``sufficient'' category.
% Our results show that ..... something about the canadian authority sites 
% (how many in each bucket, what was the natural handshake results) ...... then something about 
% how the CA authorities compared against the other sectors (comparing those same results). 
% Provide an interpretation here did canada authorities do well or not, where are they lacking, where do they excel.

% Securitytxt, the file format for vulnerability disclosure is another simple, low cost, and effective way for web administrators
% to harden their websites by allowing security researchers to easily disclose any vulnerabilities they may find. 
% This file format is recommended by the US CISA and the UK GDS.
% The results show ...... talk about CA authorities (almost no one is doing it) .... and talk about how the 
% other sectors performed (also almost no one implemented it). This demonstrates a significant gap in nearly all
% .... websites within the public and private sectors of the US, UK, and CA .... This sector perfomed the best <insert here> 
% but still didnt do well.


% Furthermore, OWASP and MDN define a set of best practice headers all websites should implement. ...... Talk very briefly
% (1-2 sentences) about each header issued, who advocates and why, then the results. The CA authority results showed ....
% reasoonable adherement to this header, ..... not so great adhere ment to that header .... The other sectors showed x y and z .....
% Canada is lacking here ..... and the other sectors are lacking here and there .... Additionally, the cookie results show gaps here
% and there for CA authorities, and the other sectors bested the CA authoritiees here and there, we did ok here .... This shows a lack
% of adherence ....

% Additionally OWASP provides guidlines for removing certain ``revealing headers'', which reveal uneeded 
% information to attackers. The results show many sites include these headers, most popular being .... likely the Server header.
% Canadian authority sites had these results, other sector sites had those results .... taken together CA needs to up their game
% in x y and z, other sectors besting the CA authorities there.


% As noted by OWASP, error pages server as another vector of revealing information for attackers. Our results of the error page
% analysis show that .... no one is leaking stack traces good ..... lots of this error leaks in CA authorities displaying the
% tech stack with almost 11 percent leaking the version ... this is not good because attackers can use the version to source
% exploits .... so and so did the best with these results .... canada authorities was ... in the pack with ....

% All in all, Canadian authorities performed ..... they need to work on this and that because of x, y and z. We are beating the other sectors
% in a, b, and c. And need to improve on d, e, and f.


\subsection{Summary of Results}

This study evaluated 375 Canadian authority origins against web security standards established by the CCCS, OWASP, and international guidance, comparing performance across sectors (education, finance, energy) and jurisdictions (Canada, US, UK). The results reveal both strengths and critical gaps in Canada's public-sector web security posture.

\subsection{Restate \& Answer Main Questions}

\textbf{RQ1: What are the key web security features and standards needed for securing Canadian websites and their visitors?}

Based on CCCS guidance (ITSP.40.062, ITSM.60.005, ITSP.40.111), OWASP recommendations, and international best practices, the essential security controls for Canadian websites comprise several layers. At the transport layer, sites must enforce HTTPS with proper HTTP-to-HTTPS redirection and implement HSTS with a minimum one-year \texttt{max-age}, \texttt{includeSubDomains}, and preferably \texttt{preload} directives. TLS configuration should prioritize version 1.3, with 1.2 acceptable using only recommended cipher suites; legacy protocols (TLS 1.1, 1.0, SSLv3) must be disabled.

At the application layer, sites should deploy security headers including \texttt{X-Content-Type-Options: nosniff} to prevent MIME-sniffing attacks, \texttt{X-Frame-Options} or CSP \texttt{frame-ancestors} directives to mitigate clickjacking, \texttt{Referrer-Policy} to control information leakage, and \texttt{Permissions-Policy} to restrict browser feature access. Cookie security requires the \texttt{Secure}, \texttt{HttpOnly}, and \texttt{SameSite} attributes on all session-related cookies.

For operational security, organizations should implement \texttt{security.txt} files per RFC~9116 to enable responsible vulnerability disclosure, remove technology-revealing headers (e.g., \texttt{Server}, \texttt{X-Powered-By}) that facilitate targeted attacks, and configure error pages to suppress stack traces and version information.

\textbf{RQ2: To what extent do Canadian public-sector and critical web services currently implement key web security controls?}

Canadian authorities demonstrate mixed compliance with these standards. For transport security, 76\% of Canadian authority origins successfully served HTTPS, with 90\% of HTTP-responding origins correctly redirecting to HTTPS. However, only 59.7\% of HTTPS-capable sites deployed HSTS, and of those, merely 46.3\% implemented strong policies meeting the one-year \texttt{max-age} threshold with subdomain coverage. This leaves a substantial portion of authority sites vulnerable to SSL-stripping attacks.

TLS configuration presents additional concerns. While 94.4\% of Canadian authorities support TLS~1.2, only 65.9\% advertise TLS~1.3 support. More troubling, 11.5\% still accept TLS~1.1 and 9.6\% accept TLS~1.0, contrary to CCCS deprecation guidance. In natural handshakes, 69.1\% negotiated TLS~1.3, with 74.9\% using recommended cipher suites when forced to TLS~1.2.

Security header adoption remains inconsistent. Canadian authorities achieved 62.3\% adoption for \texttt{X-Content-Type-Options} and 63.5\% for \texttt{X-Frame-Options}, but only 17.9\% implemented CSP \texttt{frame-ancestors}, 17.4\% set \texttt{Referrer-Policy}, and a mere 8.6\% deployed \texttt{Permissions-Policy}. Cookie security is similarly weak: among the 28.2\% of sites setting cookies, only 63.5\% used the \texttt{Secure} flag, 48.7\% used \texttt{HttpOnly}, and just 3.5\% achieved recommended configurations with all three protective attributes.

Vulnerability disclosure mechanisms are nearly absent. Only 1.1\% of Canadian authority origins (4 of 375) published a \texttt{security.txt} file, and of these, only 25\% maintained valid expiration dates. This represents a significant barrier to responsible disclosure of security vulnerabilities.

Information leakage is pervasive. Technology-revealing headers appeared on 94.4\% of Canadian authority origins, with 51.2\% exposing one header and 32.4\% exposing two. Error page analysis found 47.2\% leaked technology information, with 10.7\% exposing specific version numbers that could facilitate targeted exploitation.

\textbf{RQ3: How does adoption vary by sector and jurisdiction?}

Cross-sector analysis reveals that Canadian authorities generally underperform relative to other Canadian sectors and international counterparts.

\textit{TLS Configuration:} Canadian finance leads domestically with 94.4\% TLS~1.3 support and 89.6\% recommended cipher usage, substantially outperforming Canadian authorities (65.9\% and 74.9\%, respectively). Canadian education (77.7\% TLS~1.3, 80.4\% recommended ciphers) and energy (84.1\% TLS~1.3, 68.1\% recommended ciphers) occupy intermediate positions. Internationally, US authorities demonstrate superior TLS~1.3 adoption at 83.9\% compared to Canadian authorities' 65.9\%, with higher recommended cipher usage (85.1\% versus 74.9\%) and lower legacy protocol support.

\textit{HSTS Deployment:} Canadian authorities' 59.7\% HSTS adoption significantly trails US authorities' 89.7\%. Among Canadian sectors, finance achieves the highest HSTS presence at 78.6\%, though with lower strong policy rates (34.9\%) than authorities (46.3\%). US authorities not only lead in adoption but also in policy quality, with 64.9\% deploying strong configurations.

\textit{Security Headers:} US authorities consistently outperform Canadian authorities across all header categories: 78.0\% versus 62.3\% for \texttt{X-Content-Type-Options}, 81.2\% versus 63.5\% for \texttt{X-Frame-Options}, and 34.7\% versus 17.9\% for CSP \texttt{frame-ancestors}. Canadian energy sites demonstrate the highest domestic \texttt{Referrer-Policy} adoption at 23.7\%, compared to authorities' 17.4\%. UK finance sites lead in \texttt{Permissions-Policy} adoption at 24.0\%.

\textit{Vulnerability Disclosure:} Security.txt adoption is uniformly poor across all datasets, but Canadian authorities' 1.1\% adoption rate falls below US authorities (3.5\%), Canadian energy (4.2\%), Canadian education (3.9\%), UK education (5.8\%), and UK finance (12.1\%). This represents a critical gap in Canada's security posture.

\textit{Information Hygiene:} Canadian finance demonstrates the best domestic practice, with only 53.5\% of origins revealing technology information compared to 94.4\% for Canadian authorities. US authorities achieve better information hygiene at 80.6\% disclosure. For error page leakage, Canadian finance again leads domestically at 20.9\% technology disclosure versus 47.2\% for authorities.

\subsection{Synthesis}

Canadian authorities exhibit a two-tier security posture. Basic transport security (HTTPS availability, HTTP redirection) meets acceptable thresholds, indicating successful adoption of foundational protections. However, advanced controls---HSTS with strong policies, modern TLS versions, comprehensive security headers, cookie protections, and vulnerability disclosure mechanisms---show substantial gaps relative to both domestic regulated sectors and international government counterparts.

The finance sector's superior performance across most metrics suggests that regulatory oversight and industry standards drive security adoption more effectively than current government guidelines. The consistent outperformance of US authorities indicates that Canada's recommendation-based approach may be insufficient; the US federal government mandates HTTPS and HSTS for executive branch domains, producing measurably better outcomes.

Three priority areas emerge for immediate attention. First, HSTS deployment should be mandated with minimum one-year \texttt{max-age} and subdomain coverage; current voluntary adoption leaves 40\% of Canadian authority sites without this critical protection. Second, TLS~1.3 migration requires acceleration, with legacy protocol support (TLS~1.1/1.0) eliminated to meet CCCS guidance. Third, security.txt adoption, despite its simplicity and zero cost, remains at 1.1\%---implementing this standard would immediately improve Canada's vulnerability disclosure capabilities.
...
Several limitations constrain interpretation of these findings. First, the scan represents a single point-in-time snapshot; security configurations may change between assessments. Second, some origins were unreachable (18.6\% of Canadian authority origins), potentially biasing results if unreachable sites differ systematically from reachable ones. Third, our scanner evaluates externally observable configurations; internal security controls, incident response capabilities, and organizational security practices remain unmeasured. Fourth, UK authority data comprises only 4 origins due to domain consolidation under \texttt{gov.uk}, limiting cross-jurisdictional comparisons. Finally, classification of cipher suites as ``unknown'' (20.1\% for Canadian authorities) reflects gaps in published guidance rather than necessarily insecure configurations.
...
Canadian public-sector websites implement foundational web security controls at moderate levels but substantially lag behind both domestic regulated sectors and international government counterparts in advanced protections. The absence of mandatory security requirements, combined with reliance on recommendations rather than enforceable standards, correlates with inconsistent adoption of critical controls.
...
These measures would align Canadian government web security with international best practices and the standards-based approach advocated by the CSA Group, CCCS, and allied nations' cybersecurity agencies.
...
Future research should extend this analysis in several directions: longitudinal studies to track security posture evolution, deeper investigation of provincial and municipal government sites, assessment of authentication and access control mechanisms, and development of a Canadian-specific web security scoring framework aligned with CSA and CCCS standards. Additionally, examining the correlation between security posture and organizational factors (budget, IT staffing, outsourcing arrangements) would inform policy development.
